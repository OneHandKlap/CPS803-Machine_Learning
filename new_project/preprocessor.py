...
class Preprocessor(object):
    ...
    def create_vocabulary2(self,column,vocab_length):
        ...
        most_common=freq_dist_vocab.most_common()
        vocab_acc=[]
        for i in range(len(most_common)):
            if most_common[i][1]>5 and len(vocab_acc)<vocab_length:

                vocab_acc.append(most_common[i][0])
            elif len(vocab_acc)>=vocab_length:
                break
